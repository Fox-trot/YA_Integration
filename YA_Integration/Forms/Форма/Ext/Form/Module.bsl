...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоТест() Тогда
		Элементы.ИнтернетМагазин.Доступность	= Истина;
		Элементы.ЭтоТест.Видимость				= Истина;
		//Если Объект.ЭтоТест И ВариантЗагрузки > 0 Тогда
		//	ВыборФайлаЗавершение(ПолноеИмяФайла);
		//КонецЕсли;
	Иначе
		Объект.ЭтоТест	= Ложь;
	КонецЕсли;
...
Функция СправочникСлужбДоставкиНаСервере()
	...
	ДополнительныеПараметры = НаСервере().ПолучитьЗаголовкиЗапросаHTTP();
...
Функция ПоказатьОстаткиНаСервере(Артикул="")
	...
	ДокументРезультат.УстановитьТекст(ЗаписатьJSON1(Новый Структура("skus", ДанныеОТоварах)));
...
Процедура КомандаФайлЗагрузить(Команда=Неопределено)
	...
	Если НЕ ПроверитьЗаполнение() Тогда
		Возврат;
	ИначеЕсли Элементы.ГруппаОсновная.ТекущаяСтраница.Имя = "ГруппаСписокСоздаваемыхКарточек" Тогда
		ВариантЗагрузки = 5;
	ИначеЕсли Элементы.ГруппаОсновная.ТекущаяСтраница.Имя = "ГруппаИмпорт" Тогда
		ВариантЗагрузки = 4;
	ИначеЕсли Элементы.ГруппаОсновная.ТекущаяСтраница.Имя = "ГруппаСписокЗагруженныхКарточек" Тогда
		ВариантЗагрузки = 6;
	Иначе
	...
	ДиалогОткрытияФайла.Фильтр = ?(ВариантЗагрузки=6, "Табличный документ (*.yml)|*.yml", "Файл Microsoft Excel (*.xlsx)|*.xlsx|Табличный документ (*.mxl)|*.mxl");
...
&НаКлиенте
Процедура ВыборФайлаЗавершение(ВыбранныеФайлы=Неопределено, Контекст=Неопределено) Экспорт
	Если ТипЗнч(Контекст) = Тип("Структура") Тогда
		ДиалогОткрытияФайла = Контекст.Диалог;
		Если Не ПустаяСтрока(ДиалогОткрытияФайла.ПолноеИмяФайла) Тогда
...
&НаКлиенте
Процедура ВыполнитьЗагрузку(Результат, Адрес, ИмяФайла, ДополнительныеПараметры) Экспорт 
	Если Результат Тогда
	    ЗагрузитьНаСервере(Адрес, Сред(ИмяФайла, СтрНайти(ИмяФайла, ".", НаправлениеПоиска.СКонца) + 1));
...
Процедура Танкер5()
	...
	Пока НЕ ПустаяСтрока(ИмяКолонки) Цикл
		...
		ИначеЕсли Колонка_ЦенаБазис * Колонка_Цена * Колонка_Баркод > 0 Тогда
			Прервать;
		КонецЕсли;
		нн = нн + 1;
	КонецЦикла;
	...
	Если Колонка_Баркод > 0 Тогда
		...
		Пока нн <= ?(Объект.ЭтоТест, 99, ТабДок.ВысотаТаблицы) Цикл
			нн = нн + 1;
			//ТекСтрока	= Новый Структура("Период,Номенклатура,Характеристика", Период);
			//ТекСтрока.Вставить("Пол",		"");
			//ТекСтрока.Вставить("Цвет",		"");
			//ТекСтрока.Вставить("РосРазмер",	"");
			//ТекСтрока.Вставить("Состав",	"");
			//ТекСтрока.Вставить("ТНВЭД",		"");
			//
			//ТекСтрока.Вставить("ШтрихкодИД",	ТабДок.Область(нн, Колонка_Баркод, нн, Колонка_Баркод).Текст);
			//Если ПустаяСтрока(ТекСтрока.ШтрихкодИД) Тогда Прервать; КонецЕсли;
			//
			//ТекСтрока.Вставить("ШтрихкодИД2",	ТекСтрока.ШтрихкодИД);
			//Если НЕ НоменклатураПоБаркоду(ТекСтрока) Тогда
			//	ТекСтрока.Вставить("Ошибка",	"Штрихкод не найден");
			...
			ТекСтрока	= Новый Структура("Период", Период);
			ТекСтрока.Вставить("ШтрихкодИД",	ТабДок.Область(нн, Колонка_Баркод, нн, Колонка_Баркод).Текст);
			Если ПустаяСтрока(ТекСтрока.ШтрихкодИД) Тогда Прервать; КонецЕсли;
			
			ТекСтрока.Вставить("ШтрихкодИД2",	ТекСтрока.ШтрихкодИД);
			Виктимз.Добавить(ТекСтрока.ШтрихкодИД);
...
&НаСервере
Функция ЗатраченоВремени(Знач Сколько)
	Возврат НаСервере().ЗатраченоВремени(Сколько);
КонецФункции
...
Функция НоменклатураДописать(СтруктураПараметров)
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЕСТЬNULL(ПОДСТРОКА(Цветы.Значение, 1, 30), ЕСТЬNULL(СписокКарточек.Цвет, """")) КАК Цвет,
	|	ЕСТЬNULL(Размеры.Значение, ЕСТЬNULL(СписокКарточек.РосРазмер, """")) КАК РосРазмер,
	|	ЕСТЬNULL(СоставыТкани.НаименованиеЛокальный, ПОДСТРОКА(ЕСТЬNULL(СписокКарточек.Состав, """"), 1, 150)) КАК Состав,
	|	ЕСТЬNULL(КлассификаторТНВЭД.Код, ЕСТЬNULL(СписокКарточек.ТНВЭД, """")) КАК ТНВЭД,
	|	СписокКарточек.ИнтернетМагазин = &ИнтернетМагазин КАК Порядок
	|ИЗ
	|	РегистрСведений.Штрихкоды КАК Штрихкоды
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.мп_СписокСоздаваемыхКарточек КАК СписокКарточек
	|		ПО Штрихкоды.Характеристика = СписокКарточек.Характеристика
	|			И Штрихкоды.Владелец = СписокКарточек.Номенклатура
	...
	|				И (Цветы.ИнтернетМагазин В (&ИнтернетМагазины))
	...
	|			ПО Товар.Ссылка = КодыТНВЭД.Номенклатура
	...
	Запрос.УстановитьПараметр("ИнтернетМагазин",	Объект.ИнтернетМагазин);
	Запрос.УстановитьПараметр("ИнтернетМагазины",	ИнтернетМагазиныПолучить());
	Запрос.УстановитьПараметр("Страна",				Объект.Страна);
	//Запрос.УстановитьПараметр("Язык",				Объект.Язык);
	Выборка = Запрос.Выполнить().Выбрать();
	Пальцев	= 0;
	Пока Пальцев < 4 И Выборка.Следующий() Цикл
		Если ПустаяСтрока(СтруктураПараметров.Цвет) И НЕ ПустаяСтрока(Выборка.Цвет) Тогда
			СтруктураПараметров.Цвет = Выборка.Цвет;
			Пальцев	= Пальцев + 1;
		КонецЕсли;
		Если ПустаяСтрока(СтруктураПараметров.РосРазмер) И НЕ ПустаяСтрока(Выборка.РосРазмер) Тогда
			СтруктураПараметров.РосРазмер = Выборка.РосРазмер;
			Пальцев	= Пальцев + 1;
		КонецЕсли;
		Если ПустаяСтрока(СтруктураПараметров.Состав) И НЕ ПустаяСтрока(Выборка.Состав) Тогда
			СтруктураПараметров.Состав	= Выборка.Состав;
			Пальцев	= Пальцев + 1;
		КонецЕсли;
		Если ПустаяСтрока(СтруктураПараметров.ТНВЭД) И НЕ ПустаяСтрока(Выборка.ТНВЭД) Тогда
			СтруктураПараметров.ТНВЭД	= Выборка.ТНВЭД;
			Пальцев	= Пальцев + 1;
		КонецЕсли;
	КонецЦикла;
...
Функция ИнтернетМагазиныПолучить(ПоСтране=Ложь, ПлюсПустой=Ложь)
	//Возврат НаСервере().ИнтернетМагазиныПолучить(ПоСтране, ПлюсПустой);
	Запрос	= Новый Запрос(
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
...
&НаСервере
Функция КарточкиПоБаркоду(Баркоды)
	Запрос	= Новый Запрос(
	"ВЫБРАТЬ
	|	Товар.Ссылка КАК Номенклатура,
	|	Штрихкоды.Характеристика КАК Характеристика,
	|	МАКСИМУМ(ПОДСТРОКА(ЕСТЬNULL(Полы.Значение, СписокКарточек.Пол), 1, 10)) КАК Пол,
	|	МАКСИМУМ(ПОДСТРОКА(ЕСТЬNULL(Цветы.Значение, ЕСТЬNULL(НоменклатурныеЦвета.Значение, СписокКарточек.Цвет)), 1, 30)) КАК Цвет,
	|	МАКСИМУМ(ЕСТЬNULL(Размеры.Значение, СписокКарточек.РосРазмер)) КАК РосРазмер,
	|	МАКСИМУМ(ЕСТЬNULL(СоставыТкани.НаименованиеЛокальный, ПОДСТРОКА(СписокКарточек.Состав, 1, 150))) КАК Состав,
	|	МАКСИМУМ(ЕСТЬNULL(КлассификаторТНВЭД.Код, СписокКарточек.ТНВЭД)) КАК ТНВЭД,
	|	СписокКарточек.ИнтернетМагазин = &ИнтернетМагазин КАК Порядок,
	|	Штрихкоды.Штрихкод КАК Штрихкод
	|ИЗ
	|	РегистрСведений.Штрихкоды КАК Штрихкоды
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.мп_СписокСоздаваемыхКарточек КАК СписокКарточек
	|		ПО Штрихкоды.Характеристика = СписокКарточек.Характеристика
	|			И Штрихкоды.Владелец = СписокКарточек.Номенклатура
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК Товар
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.мп_ДанныеКарточек КАК Цветы
	...
	|				И (Цветы.Аттрибут = ЗНАЧЕНИЕ(Справочник.мп_АттрибутыКарточек.Цвет))
	|				И (Цветы.ИнтернетМагазин В (&ИнтернетМагазины))
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СловарьДанных КАК НоменклатурныеЦвета
	|			ПО Товар.Цвет = НоменклатурныеЦвета.СсылкаОбъекта
	|				И (НоменклатурныеЦвета.Язык = &Язык)
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.НоменклатурныйСоставыТкани КАК НоменклатурныйСоставыТкани
	|				ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КТ_НоменклатурныйСоставТкани КАК СоставыТкани
	|				ПО НоменклатурныйСоставыТкани.Наименование = СоставыТкани.Наименование
	|			ПО Товар.СоставТкани = НоменклатурныйСоставыТкани.Ссылка
	...
	|	Порядок УБЫВ,
	|	ТНВЭД УБЫВ");
	Запрос.УстановитьПараметр("Штрихкоды",			Баркоды);
	Запрос.УстановитьПараметр("ИнтернетМагазин",	Объект.ИнтернетМагазин);
	...
	Пока Выборка.Следующий() Цикл
		Пальцев	= 0;
		Витимз	= Объект.СписокКарточек.НайтиСтроки(Новый Структура("ШтрихкодИД", Выборка.Штрихкод));
		Для Каждого ТекСтрока Из Витимз Цикл
			...
			КонецЕсли;
			Если ПустаяСтрока(ТекСтрока.ТНВЭД) И НЕ ПустаяСтрока(Выборка.ТНВЭД) Тогда
				ТекСтрока.ТНВЭД	= Выборка.ТНВЭД;
				Пальцев	= Пальцев + 1;
			КонецЕсли;
			Если ПустаяСтрока(ТекСтрока.Состав) И НЕ ПустаяСтрока(Выборка.Состав) Тогда
				ТекСтрока.Состав	= Выборка.Состав;
				Пальцев	= Пальцев + 1;
			КонецЕсли;
			Если ПустаяСтрока(ТекСтрока.РосРазмер) И НЕ ПустаяСтрока(Выборка.РосРазмер) Тогда
				ТекСтрока.РосРазмер = Выборка.РосРазмер;
				Пальцев	= Пальцев + 1;
			КонецЕсли;
			Если ПустаяСтрока(ТекСтрока.Цвет) И НЕ ПустаяСтрока(Выборка.Цвет) Тогда
				ТекСтрока.Цвет = Выборка.Цвет;
				Пальцев	= Пальцев + 1;
			КонецЕсли;
			Если ПустаяСтрока(ТекСтрока.Пол) И НЕ ПустаяСтрока(Выборка.Пол) Тогда
				ТекСтрока.Пол	= Выборка.Пол;
				Пальцев	= Пальцев + 1;
			КонецЕсли;
			Если Пальцев = 5 Тогда Прервать; КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	Возврат (Выборка.Количество() > 0);
КонецФункции
...
&НаСервере
Функция НоменклатураПоБаркоду(СтруктураПараметров)
	Запрос	= Новый Запрос(
	"ВЫБРАТЬ
	|	Товар.Ссылка КАК Номенклатура,
	|	Штрихкоды.Характеристика КАК Характеристика,
	...
	|	ТНВЭД УБЫВ,
	|	Состав УБЫВ");
	Запрос.УстановитьПараметр("Штрихкод",			СтруктураПараметров.ШтрихкодИД);
	Запрос.УстановитьПараметр("ИнтернетМагазин",	Объект.ИнтернетМагазин);
	Запрос.УстановитьПараметр("ИнтернетМагазины",	ИнтернетМагазиныПолучить(Истина));
	Запрос.УстановитьПараметр("Страна",				Объект.Страна);
	Запрос.УстановитьПараметр("Язык",				Объект.Язык);
	...
	КонецЕсли;
	Пальцев	= 0;
	Пока Пальцев < 5 И Выборка.Следующий() Цикл
	//	Если НЕ ЗначениеЗаполнено(СтруктураПараметров.Номенклатура) Тогда
	//		СтруктураПараметров.Номенклатура	= Выборка.Номенклатура;
	//		СтруктураПараметров.Характеристика	= Выборка.Характеристика;
	//	КонецЕсли;
		Если ПустаяСтрока(СтруктураПараметров.ТНВЭД) И НЕ ПустаяСтрока(Выборка.ТНВЭД) Тогда
			СтруктураПараметров.ТНВЭД	= Выборка.ТНВЭД;
			Пальцев	= Пальцев + 1;
		...
		Если ПустаяСтрока(СтруктураПараметров.Цвет) И НЕ ПустаяСтрока(Выборка.Цвет) Тогда
			...
			Пальцев	= Пальцев + 1;
		КонецЕсли;
		Если ПустаяСтрока(СтруктураПараметров.Пол) И НЕ ПустаяСтрока(Выборка.Пол) Тогда
			СтруктураПараметров.Пол	= Выборка.Пол;
			Пальцев	= Пальцев + 1;
		КонецЕсли;
	КонецЦикла;
	Возврат (Выборка.Количество() > 0);
...
Процедура КомандаСписокКарточекЗаписать(Команда)
	Если ПроверитьЗаполнение() Тогда
		Итого = КомандаСписокКарточекЗаписатьНаСервере();
		Если Итого > 0 Тогда
			Сообщить(Строка(Итого));
			Элементы.СписокСоздаваемыхКарточек.Обновить();
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Функция КомандаСписокКарточекЗаписатьНаСервере()
	Ответ = 0;
	Для Каждого ТекСтрока Из Объект.СписокКарточек Цикл
		НЗ = РегистрыСведений.мп_СписокСоздаваемыхКарточек.СоздатьНаборЗаписей();
...
&НаСервере
Процедура Танкер6(ИмяФайла)
	Объект.Загруженные.Очистить();
	
	ЧтениеXML = Новый ЧтениеXML;
	ЧтениеXML.ОткрытьФайл(ИмяФайла);
	Пока ЧтениеXML.Прочитать() Цикл
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			Если СтрСравнить(ЧтениеXML.Имя, "categories") = 0 Тогда
				ЧтениеXML.Пропустить();
			ИначеЕсли СтрСравнить(ЧтениеXML.Имя, "offers") = 0 Тогда
				ТекСтрока = Новый Структура("Создан,offer_id,Артикул", Истина);
				Пока ЧтениеXML.Прочитать() Цикл
					Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И СтрСравнить(ЧтениеXML.Имя, "offer") = 0 Тогда
						ТекСтрока.offer_id	= ЧтениеXML.ЗначениеАтрибута("id");
						ТекСтрока.Артикул	= Лев(ТекСтрока.offer_id, 14);
					//ИначеЕсли ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И СтрСравнить(ЧтениеXML.Имя, "vendorCode") = 0 Тогда
					//	Если ЧтениеXML.Прочитать() И СтрСравнить(ЧтениеXML.Имя, "#text") = 0 Тогда
					//		ТекСтрока.Артикул	= ЧтениеXML.Значение;
					//	КонецЕсли;
					Иначе
						Продолжить;
					КонецЕсли;
					Если Объект.ЭтоТест И Объект.Загруженные.Количество() = 99 Тогда
						ЧтениеXML.Пропустить();
						
					ИначеЕсли ЗначениеЗаполнено(ТекСтрока.Артикул) И ЗначениеЗаполнено(ТекСтрока.offer_id) Тогда
						ЗаполнитьЗначенияСвойств(Объект.Загруженные.Добавить(), ТекСтрока);
...
&НаСервере
Процедура ЗагруженныеОбновить()
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Загруженные.Артикул КАК Артикул,
	|	Загруженные.offer_id КАК offer_id
	...
	|	СписокКарточек.Номенклатура КАК Номенклатура,
	|	СписокКарточек.Характеристика КАК Характеристика,
	|	СписокКарточек.Создан КАК Создан,
	|	Загруженные.Артикул КАК Артикул,
	|	Загруженные.offer_id КАК offer_id
	|ИЗ
	|	РегистрСведений.мп_СписокСоздаваемыхКарточек КАК СписокКарточек
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Загруженные КАК Загруженные
	|		ПО (СписокКарточек.Характеристика.Наименование = ПОДСТРОКА(Загруженные.offer_id, 15, 5))
	|			И СписокКарточек.Номенклатура.Артикул = Загруженные.Артикул
	|ГДЕ
	|	СписокКарточек.ИнтернетМагазин = &ИнтернетМагазин
	|
	|УПОРЯДОЧИТЬ ПО
	|	offer_id");
	Запрос.УстановитьПараметр("Загруженные",		Объект.Загруженные.Выгрузить());
	Запрос.УстановитьПараметр("ИнтернетМагазин",	Объект.ИнтернетМагазин);
	Выборка = Запрос.Выполнить().Выбрать();
	Шерлок	= Новый Структура("offer_id");
	Пока Выборка.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(Шерлок, Выборка);
		Виктимз = Объект.Загруженные.НайтиСтроки(Шерлок);
		Для Каждого ТекСтрока Из Виктимз Цикл
			ЗаполнитьЗначенияСвойств(ТекСтрока, Выборка);
		КонецЦикла;
	КонецЦикла;
	//Виктимз = Объект.Загруженные.НайтиСтроки(Новый Структура("Номенклатура,Характеристика", Справочники.Номенклатура.ПустаяСсылка(), Справочники.ХарактеристикиНоменклатуры.ПустаяСсылка()));
	//Для Каждого ТекСтрока Из Виктимз Цикл
	//	 Объект.Загруженные.Удалить(ТекСтрока);
	//КонецЦикла;
КонецПроцедуры

&НаКлиенте
Процедура КомандаКарточкиОтметитьКакСозданные(Команда)
	Если ПроверитьЗаполнение() Тогда
		КомандаКарточкиОтметитьКакСозданныеНаСервере();
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура КомандаКарточкиОтметитьКакСозданныеНаСервере()
	Для Каждого ТекСтрока Из Объект.Загруженные Цикл
		Если НЕ ТекСтрока.Создан И ЗначениеЗаполнено(ТекСтрока.Номенклатура) И ЗначениеЗаполнено(ТекСтрока.Характеристика) Тогда
			НЗ = РегистрыСведений.мп_СписокСоздаваемыхКарточек.СоздатьНаборЗаписей();
			НЗ.Отбор.Характеристика.Установить(ТекСтрока.Характеристика);
			НЗ.Отбор.Номенклатура.Установить(ТекСтрока.Номенклатура);
			НЗ.Отбор.ИнтернетМагазин.Установить(Объект.ИнтернетМагазин);
...
&НаКлиенте
Процедура КомандаТЧОчистить(Команда)
	Если Элементы.ГруппаОсновная.ТекущаяСтраница.Имя = "ГруппаЦены" Тогда
		Объект.Цены.Очистить();
	ИначеЕсли Элементы.ГруппаОсновная.ТекущаяСтраница.Имя = "ГруппаИмпорт" Тогда
		Объект.Импорт.Очистить();
	ИначеЕсли Элементы.ГруппаОсновная.ТекущаяСтраница.Имя = "ГруппаСписокСоздаваемыхКарточек" Тогда
...
Процедура КомандаТест(Команда)
	Если ПустаяСтрока(ТекстЗапроса) Тогда
		ТекстЗапроса	= ЗаписатьJSON1(Новый Структура("warehouseId,skus", warehouseId(), Топ500(Объект.ИнтернетМагазин)));
	КонецЕсли;
	Если ПроверитьЗаполнение() Тогда
		Старт		= ТекущаяУниверсальнаяДатаВМиллисекундах();
		КомандаТестНаСервере();
		Сообщить("Затрачено " + ЗатраченоВремени((ТекущаяУниверсальнаяДатаВМиллисекундах() - Старт) / 1000));
	КонецЕсли;
КонецПроцедуры
...
&НаСервереБезКонтекста
Функция Топ500(ИнтернетМагазин)
	Ответ	= Новый Массив;
	Запрос	= Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 500
	      	               |	Остатки.Номенклатура.Артикул КАК Артикул,
	      	               |	Остатки.Характеристика.Наименование КАК Размер
	      	               |ИЗ
	      	               |	РегистрСведений.ИнтернетМагазинОстатки КАК Остатки
	      	               |ГДЕ
	      	               |	Остатки.ИнтернетМагазин = &ИнтернетМагазин
	      	               |
	      	               |УПОРЯДОЧИТЬ ПО
	      	               |	Артикул,
	      	               |	Размер");
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Ответ.Добавить(Выборка.Артикул + СтрЗаменить(Выборка.Размер, " ", "/"));
	КонецЦикла;
	Возврат Ответ;
КонецФункции
...
Процедура КомандаТестНаСервере()
	...
	Источник	= "SEND_STOCK_TEST_YA";
	НаСервере	= НаСервере();
	Попытка
		ЧтениеЖсон.УстановитьСтроку(ТекстЗапроса);
		ЖС = ПрочитатьJSON(ЧтениеЖсон);
		МассивОстатков	= ПолучитьОстаткиТоваровПоSKU(ЖС.skus, ЖС.warehouseId, Объект.ИнтернетМагазин);
		Если Объект.ЭтоТест Тогда
			Результат = ЗаписатьJSON1(Новый Структура("skus", МассивОстатков));
		Иначе
			АдресРесурса	= "https://1crtlexc.koton.com.tr/live_bo_cndb/hs/exchangeYA/stocks";
			Заголовки = Новый Соответствие;
			Заголовки.Вставить("Authorization", ТокенПолучить());
			МассивДанных	= КоннекторHTTP.PostJson(АдресРесурса, ЖС, Новый Структура("Заголовки", Заголовки));
			//Если ТипЗнч(МассивДанных) = Тип("Соответствие") И ТипЗнч(МассивДанных.Получить("skus")) = Тип("Массив") Тогда
				Результат = ЗаписатьJSON1(Новый Структура("skus", МассивДанных.Получить("skus")));
			//Иначе
			//	Результат = Строка(МассивДанных);
			//КонецЕсли;
		КонецЕсли;
		//НаСервере.ЗаписатьВЖурналРегистра(НаСервере.ПолучитьПараметрИсточника(Источник), Источник, Истина, "OK");
	Исключение
...
&НаСервере
Функция ТокенПолучить()
	Ответ = "600000013B77F267";
	Запрос = Новый Запрос(
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	Параметры.Значение КАК Токен
		|ИЗ
	...
	Запрос.УстановитьПараметр("Ключ",	"Authorization");
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Ответ = Выборка.Токен;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЗаписатьJSON1(ОбъектЗаписи)
...
&НаСервереБезКонтекста
Функция ПолучитьОстаткиТоваровПоSKU(МассивSKU, warehouseId, ИнтернетМагазин)
	Выборка = ИнтеграцияWB.ПолучитьОстаткиТоваров(МассивSKU, ИнтернетМагазин);
	ДанныеОТоварах	= Новый Массив;
	ДатаФормат		= _ТекущаяДатаСеанса();
	Для Каждого ТекСтрока Из Выборка Цикл
		Остаток = Новый Структура("warehouseId, sku", warehouseId, ТекСтрока.SKU);
...
&НаСервере
Процедура КомандаОстаткиСоздатьНаСервере()
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры КАК ХарактеристикиНоменклатуры
	|		ПО Остатки.Характеристика = ХарактеристикиНоменклатуры.Ссылка
	|ГДЕ
	|	НЕ Остатки.Номенклатура В
	|				(ВЫБРАТЬ
	|					ИнтернетМагазинОстатки.Номенклатура КАК Номенклатура
	|				ИЗ
	|					РегистрСведений.ИнтернетМагазинОстатки КАК ИнтернетМагазинОстатки
	|				ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		//	НЗ.Записать();
		//КонецЕсли;
		Запись = РегистрыСведений.ИнтернетМагазинОстатки.СоздатьМенеджерЗаписи();
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	Справочник.ИнтернетМагазины КАК ИнтернетМагазины
	               |ГДЕ
	               |	ИнтернетМагазины.Действует = ИСТИНА
	               |	И ИнтернетМагазины.ПометкаУдаления = ЛОЖЬ
	               |	И ИнтернетМагазины.Ссылка <> &ИнтернетМагазин";
	ИМ = Запрос.Выполнить().Выбрать();
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	Номенклатура,
	               |	Характеристика";
	Пока ИМ.Следующий() Цикл
		Запрос.УстановитьПараметр("ИнтернетМагазин",	ИМ.ИнтернетМагазин);
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			//НЗ = РегистрыСведений.ИнтернетМагазинОстатки.СоздатьНаборЗаписей();
			//НЗ.Отбор.Характеристика.Установить(Выборка.Характеристика);
...
&НаКлиенте
Процедура КомандаОстаткиСоздать(Команда)
	Если ПроверитьЗаполнение() Тогда
		КомандаОстаткиСоздатьНаСервере();
	КонецЕсли;
КонецПроцедуры
