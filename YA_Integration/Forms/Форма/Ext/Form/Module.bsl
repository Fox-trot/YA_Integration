...
Процедура КомандаЦветаСохранитьНаСервере()
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      ...
	                      |	ПОДСТРОКА(Данные.Значение, 1, 100) КАК Значение,
	                      ...
	                      |		ПО Данные.ИнтернетМагазин = ИнтернетМагазины.Ссылка
	                      |ГДЕ
	                      |	Данные.Аттрибут = ЗНАЧЕНИЕ(Справочник.мп_АттрибутыКарточек.Цвет)
	                      |	И ИнтернетМагазины.Наименование ПОДОБНО ""%ozon%""
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	СсылкаОбъекта");
	Выборка = Запрос.Выполнить().Выбрать();
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЗ.Количество() = 0 Тогда
			...
			Если СтрЧислоВхождений(Габарит, "/") = 3 Тогда
				Габарит	= Лев(Габарит, СтрНайти(Габарит, "/",,, 3) - 1);
				Вес		= Сред(Габарит, СтрНайти(Габарит, "/",,, 3) + 1);
			...
			ТекЗапись.СсылкаОбъекта		= Выборка.СсылкаОбъекта;
			ТекЗапись.ИнтернетМагазин	= Объект.ИнтернетМагазин;
			ТекЗапись.Значение			= Выборка.Габарит;
			Попытка
				НЗ.Записать();
			Исключение
				Сообщить(ОписаниеОшибки());
