...
Функция ЭтоТест()
	Попытка
		Возврат (СтрНайти(НРег(НаСервере().ИспользуемоеИмяФайла), "\temp") = 0);
...
Процедура Танкер5()
	...
	Пока НЕ ПустаяСтрока(ИмяКолонки) Цикл
		...
		ИначеЕсли Колонка_ЦенаБазис * Колонка_Цена * (Колонка_sku + Колонка_Баркод) > 0 Тогда
			Прервать;
		КонецЕсли;
		нн = нн + 1;
	КонецЦикла;
	...
	Если Колонка_sku + Колонка_Баркод > 0 Тогда
		...
		Пока нн <= ?(Объект.ЭтоТест, 99, ТабДок.ВысотаТаблицы) Цикл
			...
			//	КонецЕсли;
			//КонецЕсли;
			//Если НЕ ЗначениеЗаполнено(ТекСтрока.Характеристика) Или НЕ ЗначениеЗаполнено(ТекСтрока.Номенклатура) Тогда
			...
			//	Прервать;
			//Иначе
				//Если НЕ НоменклатураДописать(ТекСтрока) Тогда
				//	ТекСтрока.Вставить("Ошибка",	"Атрибуты не найдены");
				//КонецЕсли;
				ЗаполнитьЗначенияСвойств(Объект.СписокКарточек.Добавить(), ТекСтрока);
			//КонецЕсли;
		КонецЦикла;
...
&НаСервере
Функция ИнтернетМагазиныПолучить()
	Запрос	= Новый Запрос(
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	И ИнтернетМагазины.Действует = ИСТИНА
...
Функция НоменклатураПоБаркоду(СтруктураПараметров)
	...
	|	ПОДСТРОКА(ЕСТЬNULL(Цветы.Значение, ЕСТЬNULL(СписокКарточек.Цвет, ЕСТЬNULL(НоменклатурныеЦвета.Значение, """"))), 1, 30) КАК Цвет,
	|	ЕСТЬNULL(Размеры.Значение, ЕСТЬNULL(СписокКарточек.РосРазмер, """")) КАК РосРазмер,
	|	ЕСТЬNULL(СоставыТкани.НаименованиеЛокальный, ПОДСТРОКА(ЕСТЬNULL(СписокКарточек.Состав, """"), 1, 150)) КАК Состав,
	|	ЕСТЬNULL(КлассификаторТНВЭД.Код, ЕСТЬNULL(СписокКарточек.ТНВЭД, """")) КАК ТНВЭД,
	|	СписокКарточек.ИнтернетМагазин = &ИнтернетМагазин КАК Порядок
