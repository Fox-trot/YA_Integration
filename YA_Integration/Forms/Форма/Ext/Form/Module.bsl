...
Функция ПоказатьОстаткиНаСервере(Артикул="")
	...
	ДокументРезультат.УстановитьТекст(ЗаписатьJSON1("skus", ДанныеОТоварах));
...
Процедура КомандаТестНаСервере()
	...
	Попытка
		...
		Если Объект.ЭтоТест Тогда
			Результат = ЗаписатьJSON1("skus", МассивОстатков);
		Иначе
			...
			Если ТипЗнч(МассивДанных) = Тип("Соответствие") И ТипЗнч(МассивДанных.Получить("skus")) = Тип("Массив") Тогда
				Результат = ЗаписатьJSON1("skus", МассивДанных.Получить("skus"));
			Иначе
...
&НаСервереБезКонтекста
Функция ЗаписатьJSON1(ИмяКлюча, МассивДанных)
	ЗаписьJSON		= Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку(Новый ПараметрыЗаписиJSON);
	ЗаписатьJSON(ЗаписьJSON, Новый Структура(ИмяКлюча, МассивДанных));
