...
Функция ПолучитьПараметрИсточника(ИмяКоманды)
...
Функция ПолучитьТаблицуЗаказовИзСайта(Финал=Ложь) Экспорт
	...
	Пока Истина Цикл
		...
		Иначе
			_error = ОшибкиПолучить(СоответствиеДанных);
			
			ЗаписатьВЖурналРегистра(ПараметрИсточник, Источник,, _error);
		    Возврат Новый Массив;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЦеныЗаполнить()
	...
	Для Итератор=2 По Макет.ВысотаТаблицы Цикл
		...
		Если ЦенаРасчетная > 0 И ЦенаБазовая > 0 Тогда
			...
			ТекСтрока.ЦенаБазовая	= ЦенаБазовая;
			ТекСтрока.ЦенаРасчетная	= ЦенаРасчетная;
	...
	Если ЭтоТест Тогда
		Цены.Свернуть("ЦенаБазовая,ЦенаРасчетная");
	КонецЕсли;
	Цены.Сортировать("ЦенаБазовая,ЦенаРасчетная");
КонецПроцедуры
...
Функция COPY_PRICELIST()
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	Шматрица.ЦенаБазовая КАК ЦенаБазовая,
		|	Шматрица.ЦенаРасчетная КАК ЦенаРасчетная
		|ПОМЕСТИТЬ Шматрица
		|ИЗ
		|	&Цены КАК Шматрица
		|;
...
Функция КарточкиПолучить()
	...
	|	ЕСТЬNULL(ПОДСТРОКА(Цветы.Значение, 1, 100), ЕСТЬNULL(КТ_НоменклатурныеЦвета.Наименование, """")) КАК Цвет,
	|	Штрихкоды.Штрихкод КАК barcode,
	|	Штрихкоды.Характеристика КАК Характеристика,
	|	Штрихкоды.Характеристика.Наименование КАК ХарактеристикаНаименование,
	...
	|				ПО Товар.Ссылка = Штрихкоды.Владелец
	|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(, ВидЦены = &ВидЦены) КАК ЦеныНоменклатуры
	|				ПО Товар.Ссылка = ЦеныНоменклатуры.Номенклатура
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеРеквизитовИМ КАК Реквизиты
	|					ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КТ_НоменклатурныйСоставТкани КАК СоставыТкани
	|					ПО Реквизиты.СоставТкани = СоставыТкани.Ссылка
	|					ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КТ_НоменклатурныеЦвета КАК КТ_НоменклатурныеЦвета
	|					ПО Реквизиты.Цвет = КТ_НоменклатурныеЦвета.Ссылка
	|				ПО Товар.Ссылка = Реквизиты.Номенклатура
	...
	|			ПО Категории._MopClass = Товар._MopClass
	|				И Категории._MopDepartment = Товар._MopDepartment
	|				И Категории._MopSubDepartment = Товар._MopSubDepartment
	|				И Категории._MopCategory = Товар._MopCategory
	|				И Категории._MopDivision = Товар._MopDivision
	|		ПО Данные.СсылкаОбъекта = Категории.Ссылка
	|ГДЕ
	...
	|	И Типы.Аттрибут = ЗНАЧЕНИЕ(Справочник.мп_АттрибутыКарточек.Тип)
	|	И Товар.ПометкаУдаления = ЛОЖЬ
	|	И ЕСТЬNULL(ПОДСТРОКА(Цветы.Значение, 1, 100), ЕСТЬNULL(КТ_НоменклатурныеЦвета.Наименование, """")) > "" ""
	|
	|УПОРЯДОЧИТЬ ПО
	|	category,
	...
	//Запрос.УстановитьПараметр("Обязательные",		АттрибутыОбязательные);
	//Запрос.УстановитьПараметр("Дата",				ТекущаяДатаСеанса());
	Запрос.УстановитьПараметр("ВидЦены",			ВидЦены);
	Запрос.УстановитьПараметр("Страна",				Страна);
...
Функция ХарактеристикиПолучить(Номенклатура)
	Запрос	= Новый Запрос(
	"ВЫБРАТЬ
	|	ЕСТЬNULL(СтраныПроисхождения.Значение, """") КАК country_of_origin,
	|	ЕСТЬNULL(Полы.Значение, """") КАК Пол,
	|	Реквизиты.Изображение1 КАК Изображение1,
	|	Реквизиты.Изображение2 КАК Изображение2,
	|	Реквизиты.Изображение3 КАК Изображение3,
	|	Реквизиты.Изображение4 КАК Изображение4,
	|	Реквизиты.Изображение5 КАК Изображение5,
	|	Реквизиты.Изображение6 КАК Изображение6,
	|	Реквизиты.Изображение7 КАК Изображение7,
	|	Реквизиты.Изображение8 КАК Изображение8,
	|	Реквизиты.Изображение9 КАК Изображение9,
	|	Реквизиты.Изображение10 КАК Изображение10
	|ИЗ
	|	Справочник.Номенклатура КАК Товар
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СоответствиеРеквизитовИМ КАК Реквизиты
	|		ПО Товар.Ссылка = Реквизиты.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.мп_ДанныеКарточек КАК СтраныПроисхождения
	|		ПО Товар.СтранаПроисхождения = СтраныПроисхождения.СсылкаОбъекта
	|			И (СтраныПроисхождения.ИнтернетМагазин = &ИнтернетМагазин)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.мп_ДанныеКарточек КАК Полы
	|		ПО Товар.Пол = Полы.СсылкаОбъекта
	|			И (Полы.ИнтернетМагазин = &ИнтернетМагазин)
	|ГДЕ
...
Функция РазмерПолучить(Номенклатура, Знач Размер)
	Запрос	= Новый Запрос(
	"ВЫБРАТЬ
	|	Размеры.Значение КАК Значение
	|ИЗ
	|	Справочник.Номенклатура КАК Номенклатура
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИнтернетМагазинРазмеры КАК Размеры
	|		ПО Номенклатура._MopClass = Размеры.Категория
	|			И Номенклатура._MopSubDepartment = Размеры.Пол
	|ГДЕ
	|	Номенклатура.Ссылка = &Номенклатура
	|	И Размеры.ИнтернетМагазин = &ИнтернетМагазин
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Размер = Выборка.Значение;
	КонецЕсли;
	Возврат Размер;
КонецФункции

Процедура ЗаписатьТекст(ЗаписьХМL, ПолноеИмя, Текст)
	...
	ЗаписьХМL.ЗаписатьАтрибут("date", Формат(ТекущаяДатаСеанса(), "ДФ='yyyy-MM-dd HH:mm'"));
		ЗаписьХМL.ЗаписатьНачалоЭлемента("shop");
			...
			Пока Карточки.Следующий() Цикл
				Если categoryID <> Карточки.categoryID Тогда
					categoryID  = Карточки.categoryID;
					ЗаполнитьЗначенияСвойств(categories.Добавить(), Карточки);
				КонецЕсли;
				Если мСсылкаОбъекта.Найти(Карточки.СсылкаОбъекта) = Неопределено Тогда
					мСсылкаОбъекта.Добавить(Карточки.СсылкаОбъекта);
				КонецЕсли;
				
				ЗаписьХМL.ЗаписатьНачалоЭлемента("offer");
				ЗаписьХМL.ЗаписатьАтрибут("id",	Карточки.Артикул + Карточки.ХарактеристикаНаименование);
					ЗаписатьТекст(ЗаписьХМL, "name", Карточки.Тип + " " + Котон() + " " + Карточки.Цвет);
					...
					ЗаписатьТекст(ЗаписьХМL, "vendor", Котон());
					ЗаписатьТекст(ЗаписьХМL, "vendorCode", Карточки.Артикул);
					ЗаписатьТекст(ЗаписьХМL, "price", Формат(Карточки.price, "ЧГ=0"));
					//Если Выборка.oldprice > Выборка.price Тогда
					//	ЗаписьХМL.ЗаписатьНачалоЭлемента("oldprice");
					//	ЗаписьХМL.ЗаписатьТекст(Выборка.oldprice);
					//	ЗаписьХМL.ЗаписатьКонецЭлемента();
					//КонецЕсли;
					ЗаписатьТекст(ЗаписьХМL, "barcode", Формат(Карточки.barcode, "ЧГ=0"));
					//ЗаписатьТекст(ЗаписьХМL, "description", Товар.category + Карточки.);
					
					ЗаписьХМL.ЗаписатьНачалоЭлемента("param");
					ЗаписьХМL.ЗаписатьАтрибут("name", "Цвет");
					ЗаписьХМL.ЗаписатьТекст(Карточки.Цвет);
					ЗаписьХМL.ЗаписатьКонецЭлемента();
					ЗаписьХМL.ЗаписатьНачалоЭлемента("param");
					ЗаписьХМL.ЗаписатьАтрибут("name", "Состав");
					ЗаписьХМL.ЗаписатьТекст(СокрЛП(Карточки.Состав));
					ЗаписьХМL.ЗаписатьКонецЭлемента();
					
					Если Характеристики = Неопределено Тогда
						Номенклатура = Карточки.Номенклатура;
						Характеристики = ХарактеристикиПолучить(Карточки.Номенклатура);
					ИначеЕсли Номенклатура = Карточки.Номенклатура Тогда
						Характеристики.Сбросить();
					Иначе
						Номенклатура = Карточки.Номенклатура;
						Характеристики = ХарактеристикиПолучить(Карточки.Номенклатура);
					КонецЕсли;
					Если Характеристики.Следующий() Тогда
						ЗаписьХМL.ЗаписатьНачалоЭлемента("param");
						ЗаписьХМL.ЗаписатьАтрибут("name", "Размер");
						ЗаписьХМL.ЗаписатьТекст(РазмерПолучить(Карточки.Номенклатура, Карточки.ХарактеристикаНаименование));
						ЗаписьХМL.ЗаписатьКонецЭлемента();
						Если НЕ ПустаяСтрока(Характеристики.Пол) Тогда
							ЗаписьХМL.ЗаписатьНачалоЭлемента("param");
							ЗаписьХМL.ЗаписатьАтрибут("name", "Пол");
							ЗаписьХМL.ЗаписатьТекст(Характеристики.Пол);
							ЗаписьХМL.ЗаписатьКонецЭлемента();
						КонецЕсли;
						...
						Если НЕ ПустаяСтрока(Характеристики.Изображение6) Тогда
						...
						КонецЕсли;
						//ЗаписьХМL.ЗаписатьНачалоЭлемента("weight");
						//ЗаписьХМL.ЗаписатьТекст("1");
						//ЗаписьХМL.ЗаписатьКонецЭлемента();
						//ЗаписьХМL.ЗаписатьНачалоЭлемента("length");
						//ЗаписьХМL.ЗаписатьТекст("2");
						//ЗаписьХМL.ЗаписатьКонецЭлемента();
						//ЗаписьХМL.ЗаписатьНачалоЭлемента("width");
						//ЗаписьХМL.ЗаписатьТекст("3");
						//ЗаписьХМL.ЗаписатьКонецЭлемента();
						
						Если НЕ ПустаяСтрока(Характеристики.country_of_origin) Тогда
							ЗаписьХМL.ЗаписатьНачалоЭлемента("country_of_origin");
							ЗаписьХМL.ЗаписатьТекст(Характеристики.country_of_origin);
							ЗаписьХМL.ЗаписатьКонецЭлемента();
						КонецЕсли;
					КонецЕсли;
					ЗаписьХМL.ЗаписатьНачалоЭлемента("tn-ved-codes");
						ЗаписьХМL.ЗаписатьНачалоЭлемента("tn-ved-code");
						ЗаписьХМL.ЗаписатьТекст(Карточки.ТНВЭД);
						ЗаписьХМL.ЗаписатьКонецЭлемента();
					ЗаписьХМL.ЗаписатьКонецЭлемента();
				ЗаписьХМL.ЗаписатьКонецЭлемента();
			КонецЦикла;
			...
			//////////////////////////////////////////////////////////////////////////////////////////////
			categoriesПолучить(мСсылкаОбъекта);
			...
			Для Каждого ТекСтрока Из categories Цикл
				ЗаписьХМL.ЗаписатьНачалоЭлемента("category");
					ЗаписьХМL.ЗаписатьАтрибут("id", ТекСтрока.categoryID);
					ЗаписьХМL.ЗаписатьТекст(ТекСтрока.category);
				ЗаписьХМL.ЗаписатьКонецЭлемента();
			КонецЦикла;
	        ЗаписьХМL.ЗаписатьКонецЭлемента();
		ЗаписьХМL.ЗаписатьКонецЭлемента();
...
Процедура categoriesПолучить(Ссылки)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      |	ПОДСТРОКА(ДанныеКарточек.Значение, 1, 100) КАК categoryID,
	                      |	ПОДСТРОКА(мп_ДанныеКарточек.Значение, 1, 100) КАК category
	                      |ИЗ
	                      |	РегистрСведений.мп_ДанныеКарточек КАК ДанныеКарточек
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.мп_ДанныеКарточек КАК мп_ДанныеКарточек
	                      |		ПО ДанныеКарточек.СсылкаОбъекта = мп_ДанныеКарточек.СсылкаОбъекта
	                      |			И ДанныеКарточек.ИнтернетМагазин = мп_ДанныеКарточек.ИнтернетМагазин
	                      |ГДЕ
	                      |	ДанныеКарточек.Аттрибут = ЗНАЧЕНИЕ(Справочник.мп_АттрибутыКарточек.КомплектацияID)
	                      |	И ДанныеКарточек.ИнтернетМагазин В(&ИнтернетМагазины)
	                      |	И ДанныеКарточек.СсылкаОбъекта В(&Ссылки)
	                      |	И мп_ДанныеКарточек.Аттрибут = ЗНАЧЕНИЕ(Справочник.мп_АттрибутыКарточек.Комплектация)
	                      |
	                      |ОБЪЕДИНИТЬ
	                      |
	                      |ВЫБРАТЬ РАЗЛИЧНЫЕ
	                      |	ПОДСТРОКА(ДанныеКарточек.Значение, 1, 100),
	                      |	ПОДСТРОКА(мп_ДанныеКарточек.Значение, 1, 100)
	                      |ИЗ
	                      |	РегистрСведений.мп_ДанныеКарточек КАК ДанныеКарточек
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.мп_ДанныеКарточек КАК мп_ДанныеКарточек
	                      |		ПО ДанныеКарточек.СсылкаОбъекта = мп_ДанныеКарточек.СсылкаОбъекта
	                      |			И ДанныеКарточек.ИнтернетМагазин = мп_ДанныеКарточек.ИнтернетМагазин
	                      |ГДЕ
	                      |	ДанныеКарточек.Аттрибут = ЗНАЧЕНИЕ(Справочник.мп_АттрибутыКарточек.КатегорияID)
	                      |	И ДанныеКарточек.ИнтернетМагазин В(&ИнтернетМагазины)
	                      |	И ДанныеКарточек.СсылкаОбъекта В(&Ссылки)
	                      |	И мп_ДанныеКарточек.Аттрибут = ЗНАЧЕНИЕ(Справочник.мп_АттрибутыКарточек.Категория)
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	categoryID");
	...
	ИнтернетМагазины.Добавить(ИнтернетМагазин);
	ИнтернетМагазины.Добавить(Справочники.ИнтернетМагазины.ПустаяСсылка());
	Запрос.УстановитьПараметр("ИнтернетМагазины",	ИнтернетМагазины);
	Выборка = Запрос.Выполнить().Выбрать();
	categories.Очистить();
	Пока Выборка.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(categories.Добавить(), Выборка);
	КонецЦикла;
КонецПроцедуры
