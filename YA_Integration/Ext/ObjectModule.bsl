...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	ПараметрыРегистрации.Версия = "1.14";
...
Функция КарточкиПолучить()
	...
	|	МАКСИМУМ(НоменклатурныеЦвета.КодИсточник) КАК ЦветКод,
	...
	|	МАКСИМУМ(СоставыТкани.НаименованиеЛокальный) КАК Состав,
	...
	|	СписокКарточек.Характеристика.Наименование,
...
Функция РосРазмерПолучить(Номенклатура, Знач Размер)
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ИнтернетМагазины.Страна = &Страна УБЫВ");
	Запрос.УстановитьПараметр("ИнтернетМагазин",	ИнтернетМагазинЯндекс());
	Запрос.УстановитьПараметр("Номенклатура",		Номенклатура);
...
Функция НаименованиеПолучить(Карточки, РосРазмер)
	...
	Если ПустаяСтрока(РосРазмер) Или Карточки.ХарактеристикаНаименование = РосРазмер Тогда
	...
	ЗаписьХМL.ЗаписатьАтрибут("date", Формат(ТекущаяДатаСеанса(), "ДФ='yyyy-MM-dd HH:mm'"));
		ЗаписьХМL.ЗаписатьНачалоЭлемента("shop");
			...
			Пока Карточки.Следующий() Цикл
				...
				//РосРазмер	= РосРазмерПолучить(Карточки.Номенклатура, Карточки.ХарактеристикаНаименование);
				//Гугл		= Новый Структура("ЯндексSKU,Размеры", Карточки.Артикул, РосРазмер);
				...
				ЗаписьХМL.ЗаписатьАтрибут("id",	СокрЛП(Карточки.Артикул) + СтрЗаменить(СокрЛП(Карточки.ХарактеристикаНаименование), " ", "/"));
					name	= Карточки.Модель + " " + Карточки.ХарактеристикаНаименование + ?(ПустаяСтрока(Карточки.РосРазмер), "", "(" + СокрЛП(Карточки.РосРазмер) + ")") + " " + Карточки.ЦветКод + " " + Карточки.Цвет;
					//name	= Карточки.Модель + " " + Карточки.ХарактеристикаНаименование + ?(ПустаяСтрока(РосРазмер), "", "(" + СокрЛП(РосРазмер) + ")") + " " + Карточки.ЦветКод + " " + Карточки.Цвет;
