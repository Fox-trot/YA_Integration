...
Функция ХарактеристикиПолучить(Номенклатура)
	...
	|	ЕСТЬNULL(СоставыТкани.НаименованиеЛокальный, """") КАК Состав,
	...
	|	ЕСТЬNULL(КлассификаторТНВЭД.Код, """") КАК ТНВЭД
	|ИЗ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КодыТНВЭД.СрезПоследних(
	|				,
	|				Номенклатура = &Номенклатура
	|					И Страна = &Страна) КАК КодыТНВЭД
