...
Функция КарточкиПолучить()
	...
	|	ПОДСТРОКА(Цветы.Значение, 1, 100) КАК Цвет,
	|	Штриходы.ШтрихкодИД КАК barcode,
	|	Штриходы.Характеристика КАК Характеристика,
	|	Штриходы.Характеристика.Наименование КАК ХарактеристикаНаименование,
	...
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИнтернетМагазинШтриходы КАК Штриходы
	|				ПО Товар.Ссылка = Штриходы.Номенклатура
	...
	Если НЕ ЭтоТест Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПЕРВЫЕ 7", "");
	КонецЕсли;
...
Функция ХарактеристикиПолучить(Номенклатура)
	...
	|	ЕСТЬNULL(КТ_НоменклатурныйСоставТкани.НаименованиеЛокальный, """") КАК Состав,
	|	ЕСТЬNULL(Реквизиты.Изображение1, """") КАК Изображение1,
	|	ЕСТЬNULL(Реквизиты.Изображение2, """") КАК Изображение2,
	|	ЕСТЬNULL(Реквизиты.Изображение3, """") КАК Изображение3,
	|	ЕСТЬNULL(Реквизиты.Изображение4, """") КАК Изображение4,
	|	ЕСТЬNULL(Реквизиты.Изображение5, """") КАК Изображение5,
	|	ЕСТЬNULL(Реквизиты.Изображение6, """") КАК Изображение6,
	|	ЕСТЬNULL(Реквизиты.Изображение7, """") КАК Изображение7,
	|	ЕСТЬNULL(Реквизиты.Изображение8, """") КАК Изображение8,
	|	ЕСТЬNULL(Реквизиты.Изображение9, """") КАК Изображение9,
	|	ЕСТЬNULL(Реквизиты.Изображение10, """") КАК Изображение10
	|ИЗ
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КТ_НоменклатурныйСоставТкани КАК КТ_НоменклатурныйСоставТкани
	|			ПО Реквизиты.СоставТкани = КТ_НоменклатурныйСоставТкани.Ссылка
	|				И (КТ_НоменклатурныйСоставТкани.Язык = &Язык)
		...
		ЗаписьХМL.ЗаписатьНачалоЭлемента("shop");
			...
			Пока Карточки.Следующий() Цикл
				...
				ЗаписьХМL.ЗаписатьАтрибут("id",	Карточки.Артикул + Карточки.ХарактеристикаНаименование);
					...
					//ЗаписьХМL.ЗаписатьНачалоЭлемента("market-sku");
					//ЗаписьХМL.ЗаписатьТекст(Выборка.Артикул + Выборка.ХарактеристикаНаименование);
					//ЗаписьХМL.ЗаписатьКонецЭлемента();
					
					ЗаписатьТекст(ЗаписьХМL, "categoryId", Карточки.categoryID);
					//ЗаписьХМL.ЗаписатьНачалоЭлемента("description");
					//ЗаписьХМL.ЗаписатьТекст(Строка(Товар.category)+ Строка(Товар.));
					//ЗаписьХМL.ЗаписатьКонецЭлемента();
					...
					Если Характеристики.Следующий() Тогда
						...
						ЗаписьХМL.ЗаписатьТекст(СокрЛП(Характеристики.Состав));
						ЗаписьХМL.ЗаписатьКонецЭлемента();
						...
						//	ЗаписьХМL.ЗаписатьТекст(Характеристики.ТНВЭД);
						//	ЗаписьХМL.ЗаписатьКонецЭлемента();
						//ЗаписьХМL.ЗаписатьКонецЭлемента();
			...
			Для Каждого ТекСтрока Из categories Цикл
				ЗаписьХМL.ЗаписатьНачалоЭлемента("category");
					ЗаписьХМL.ЗаписатьАтрибут("id", Карточки.categoryID);
					ЗаписьХМL.ЗаписатьТекст(Карточки.category);
				ЗаписьХМL.ЗаписатьКонецЭлемента();
			КонецЦикла;
	        ЗаписьХМL.ЗаписатьКонецЭлемента();
		ЗаписьХМL.ЗаписатьКонецЭлемента();
