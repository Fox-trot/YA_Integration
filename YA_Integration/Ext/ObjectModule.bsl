...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	ПараметрыРегистрации.Версия = "1.13";
...
Функция КарточкиПолучить()
	...
	|	СписокКарточек.Характеристика.Наименование КАК ХарактеристикаНаименование,
...
Процедура ХарактеристикиПолучить(Параметры, Характеристики)
	...
	|	ЕСТЬNULL(Полы.Значение, КТ_НоменклатурныеРазделения.Наименование) КАК Пол,
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КТ_НоменклатурныеРазделения КАК КТ_НоменклатурныеРазделения
	|			ПО Реквизиты.Разделение = КТ_НоменклатурныеРазделения.Ссылка
	...
	|				ИЛИ СтраныПроисхождения.ИнтернетМагазин.Язык = &Язык)
		...
		ЗаписьХМL.ЗаписатьНачалоЭлемента("shop");
			...
			Для Каждого ТекСтрока Из categories Цикл
				Если ТекСтрока.parentId > 0 И НЕ ПустаяСтрока(ТекСтрока.parent) Тогда
					Если parentId.Найти(ТекСтрока.parentId) = Неопределено Тогда
						parentId.Добавить(ТекСтрока.parentId);
						ЗаписьХМL.ЗаписатьНачалоЭлемента("category");
						ЗаписьХМL.ЗаписатьАтрибут("id", Формат(ТекСтрока.parentId, "ЧГ=0"));
						ЗаписьХМL.ЗаписатьТекст(ТекСтрока.parent);
						ЗаписьХМL.ЗаписатьКонецЭлемента();
					КонецЕсли;
				КонецЕсли;
				ЗаписьХМL.ЗаписатьНачалоЭлемента("category");
			...
			Пока Карточки.Следующий() Цикл
				...
				Размер	= РазмерПолучить(Карточки.Номенклатура, Карточки.ХарактеристикаНаименование);
				Гугл	= Новый Структура("ЯндексSKU,Размеры", Карточки.Артикул, Размеры);
				Если Размеры.НайтиСтроки(Гугл).Количество() > 0 Тогда
					Продолжить;
				КонецЕсли;
				ЗаполнитьЗначенияСвойств(Размеры.Добавить(), Гугл);
				
				ЗаписьХМL.ЗаписатьАтрибут("id",	Карточки.Артикул + Карточки.ХарактеристикаНаименование);
					ЗаписатьТекст(ЗаписьХМL, "name", Карточки.category + " " + Котон() + " " + Карточки.Цвет);
					...
					//ЗаписатьТекст(ЗаписьХМL, "description", Карточки.description);
					...
					КонецЕсли;
					Если Характеристики.Номенклатура = Карточки.Номенклатура Тогда
						ЗаписатьТекст(ЗаписьХМL, "Размер", Размер, Истина);
				...
				//	НЗ.Отбор.ИнтернетМагазин.Установить(Карточки.ИнтернетМагазин);
				//	НЗ.Прочитать();
				//	Для Каждого ТекЗапись Из НЗ Цикл
...
Процедура categoriesПолучить(Ссылки)
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПОДСТРОКА(КатегорииИД.Значение, 1, 100) КАК categoryID,
	|	ПОДСТРОКА(Категории.Значение, 1, 100) КАК category,
	|	ПОДСТРОКА(КомплектацияИД.Значение, 1, 100) КАК parentId,
