...
Функция КарточкиПолучить()
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если ЭтоТест() Тогда
		Сообщить("Выполнено за " + Время((ТекущаяУниверсальнаяДатаВМиллисекундах() - Старт) / 1000));
...
Процедура ХарактеристикиПолучить(Параметры, Характеристики)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.мп_СписокСоздаваемыхКарточек КАК Карточки
	|		ПО Реквизиты.Номенклатура = Карточки.Номенклатура
	|ГДЕ
	|	Реквизиты.Номенклатура = &Номенклатура
	|	И Реквизиты.ИнтернетМагазин В(&ИнтернетМагазины)
	|	И Карточки.ИнтернетМагазин В(&ИнтернетМагазины)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Реквизиты.ИнтернетМагазин = &ИнтернетМагазин УБЫВ,
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли НЕ ПустаяСтрока(Характеристики.Изображение10)
...
Функция CARDS_CREATE() Экспорт
	...
	Если ЭтоТест() Тогда
		Сообщить("Затрачено " + Время((ТекущаяУниверсальнаяДатаВМиллисекундах() - Старт) / 1000));
	КонецЕсли;
	Возврат ИмяФайла;
КонецФункции
...
Функция ЭтоТест()
	Попытка
		Возврат (СтрНайти(НРег(ИспользуемоеИмяФайла), "\temp") = 0);
	Исключение КонецПопытки;
	Возврат Ложь;
КонецФункции

Функция Время(Знач Сколько)
	Если Сколько < 1 Тогда Возврат "0 сек"; КонецЕсли;
	Ответ = "";
	Если Сколько >= 86400 Тогда
		Ответ	= Формат(Цел(Сколько / 86400), "ЧГ=0") + " дн";
		Сколько	= Сколько - Цел(Сколько / 86400) * 86400;
	КонецЕсли;
	Если Сколько >= 3600 Тогда
		Ответ	= СокрЛП(Ответ + " " + Формат(Цел(Сколько / 3600), "ЧГ=0") + " час");
		Сколько	= Сколько - Цел(Сколько / 3600) * 3600;
	КонецЕсли;
	Если Сколько >= 60 И СтрЧислоВхождений(Ответ, " ") < 2 Тогда
		Ответ	= СокрЛП(Ответ + " " + Формат(Цел(Сколько / 60), "ЧГ=0") + " мин");
		Сколько	= Сколько - Цел(Сколько / 60) * 60;
	КонецЕсли;
	Если Цел(Сколько) > 0 И СтрЧислоВхождений(Ответ, " ") < 2 Тогда
		Ответ =СокрЛП(Ответ + " " + Формат(Сколько, "ЧДЦ=0; ЧГ=0") + " сек");
	КонецЕсли;
	Возврат Ответ;
КонецФункции
