		...
		ЗаписьХМL.ЗаписатьНачалоЭлемента("shop");
			...
			Для Каждого ТекСтрока Из categories Цикл
				ЗаписьХМL.ЗаписатьНачалоЭлемента("category");
					...
					Если parentId > 0 Тогда
						ЗаписьХМL.ЗаписатьАтрибут("parentId", Формат(parentId, "ЧГ=0"));
					КонецЕсли;
					ЗаписьХМL.ЗаписатьТекст(ТекСтрока.category);
				ЗаписьХМL.ЗаписатьКонецЭлемента();
				
				parentId	= ТекСтрока.parentId;
			КонецЦикла;
	        ЗаписьХМL.ЗаписатьКонецЭлемента();
		ЗаписьХМL.ЗаписатьКонецЭлемента();
	ЗаписьХМL.ЗаписатьКонецЭлемента();
...
Процедура categoriesПолучить(Ссылки)
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(ТекСтрока, Выборка);
		ТекСтрока.parentId	= СтрокаВЧисло(Выборка.categoryID);
	КонецЦикла;
	categories.Сортировать("parentId");
КонецПроцедуры
